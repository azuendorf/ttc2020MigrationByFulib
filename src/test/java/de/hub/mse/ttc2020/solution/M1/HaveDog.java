...
public class HaveDog extends ModelCommand
{
   ...
   {
      EObject dog = editor.getOrCreateDog(getId());
      EClass dogClass = (EClass) editor.getEmfModel().getEClassifier("Dog");
      dog.eSet(dogClass.getEStructuralFeature("name"), name == null ? "" : name);
      EStructuralFeature ageFeature = dogClass.getEStructuralFeature("age");
      if (ageFeature != null) {
         dog.eSet(ageFeature, this.age);
      }

      EObject person = editor.getOrCreatePerson(owner);
      EStructuralFeature ownerFeature = dogClass.getEStructuralFeature("owner");
      dog.eSet(ownerFeature, person);
