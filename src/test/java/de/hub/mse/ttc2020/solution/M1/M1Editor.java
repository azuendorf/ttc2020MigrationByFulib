...
public class M1Editor 
{
   ...
   public M1Editor setEmfModel(EPackage emfModel)
   {
   ...
   public M1Editor parse(EObject instance)
   {
   ...
   private void parsePerson(EObject person)
   {
      if (person == null) {
         System.out.println();
      }
      String id = getPersonId(person);
      HavePerson oldCommand = (HavePerson) activeCommands.get(id);

      EClass class1 = person.eClass();
      String name = (String) person.eGet(class1.getEStructuralFeature("name"));
      if ("".equals(name) && oldCommand != null && oldCommand.getName() == null) {
         name = null;
      }
      ...
      EStructuralFeature ageFeature = class1.getEStructuralFeature("age");
      if (ageFeature != null) {
         age = (Integer) person.eGet(ageFeature);
      }
      else {
         if (oldCommand != null) {
            age = oldCommand.getAge();
   ...
   public void apply(String yaml)
   {
      ...
      for (Object m2Command : commandList.values()) {
         execute((ModelCommand) m2Command);
      }
   }
